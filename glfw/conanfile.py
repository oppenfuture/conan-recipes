...
class GlfwConan(ConanFile):
    ...
    options = {"shared": [True, False], "fPIC": [True, False], 'mesa': [True, False]}
    default_options = {'shared': False, "fPIC": True, 'mesa': False}
    license = "Zlib"
    ...
    def _configure_cmake(self):
        cmake = CMake(self)
        cmake.definitions["GLFW_BUILD_EXAMPLES"] = True
        cmake.definitions["GLFW_BUILD_TESTS"] = True
    ...
    def package_info(self):
        ...
        if self.settings.os == "Linux":
            self.cpp_info.system_libs.extend(['m', 'dl', 'pthread', 'X11'])
            if self.options.shared:
                self.cpp_info.exelinkflags.append("-lrt -lm -ldl -lX11")
